/*
    AtCoder —ABC228.F —二维线段树，区间最大/最小值，单点修改，O(nmlognlogm)
*/
#include <bits/stdc++.h>
...
inline void solve() {
    ...
    for (int i = 1; i + h - 1 <= n; i++)
        for (int j = 1; j + w - 1 <= m; j++)
            fff.update(i, j, sum[i + h - 1][j + w - 1] - sum[i - 1][j + w - 1] - sum[i + h - 1][j - 1] + sum[i - 1][j - 1]);

    ll mx = -INF;

    for (int i = 1; i + h1 - 1 <= n; i++)
        for (int j = 1; j + w1 - 1 <= m; j++)
            mx = max(mx, sum[i + h1 - 1][j + w1 - 1] - sum[i - 1][j + w1 - 1] - sum[i + h1 - 1][j - 1] + sum[i - 1][j - 1]
